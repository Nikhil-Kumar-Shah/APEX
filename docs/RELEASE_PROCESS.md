...
2. **Version Bump**:
   - Update `__version__` in [runtime/__init__.py](file:///d:/APEX/runtime/__init__.py).
   - Update `version` in [pyproject.toml](file:///d:/APEX/pyproject.toml).
   - Update `LATEST_RUNTIME_VERSION` in [updater.py](file:///d:/APEX/runtime/ui/updater.py).
3. **Verify Configuration Mappings**: Ensure that example configurations under `examples/` match the active schemas.
4. **Update Changelog**: Add release notes, dates, and changes to [CHANGELOG.md](file:///d:/APEX/CHANGELOG.md).
